...
from src.database.repository import (
    create_dispute, get_dispute, get_all_disputes, get_dispute_timeline, get_dispute_case_analysis,
...
from src.schemas.api_schemas import (
    ...
    DisputeWorkflowTransitionSchema, DisputeCaseReadinessSchema, DisputeSubmitResponseSchema,
    AIExplainabilitySchema, NextBestActionSchema, DisputeAuditEventSchema, ChargebackPackageInspectionSchema,
...
def list_disputes_endpoint(case_source: Optional[str] = None, db: Session = Depends(get_db)):
    """Retrieves dispute records from database with deadline metadata. Optionally filters by case_source."""
    disputes = get_all_disputes(db)
    if case_source:
        disputes = [d for d in disputes if getattr(d, "case_source", "SIMULATED_RAZORPAY") == case_source]
...
def create_dispute_endpoint(payload: DisputeCreateSchema, db: Session = Depends(get_db)):
    ...
    try:
        ...
        dispute = create_dispute(db, data_dict)
...
def submit_dispute_endpoint(dispute_id: str, db: Session = Depends(get_db)):
    """
    Executes the hard backend submission gate pipeline.
    Validates readiness, advances workflow_stage to SUBMITTED, and records local gateway boundary submission.
    """
    try:
        result = submit_dispute_package(db, dispute_id)
        return result
    except ValueError as ve:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
