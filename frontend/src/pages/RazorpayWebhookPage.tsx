import React, { useState, useEffect } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import { simulationService, SimulateDisputeResponse } from '../services/simulationService';
import { SimulationTransaction } from '../types/simulation';
import { Card } from '../components/common/Card';
import { Button } from '../components/common/Button';
import { Modal } from '../components/common/Modal';
import { Skeleton } from '../components/common/Skeleton';
import { formatCurrency, formatDate } from '../utils/formatters';
import { useDatabaseMode } from '../context/DatabaseModeContext';
import { useRealtimeEvents } from '../hooks/useRealtimeEvents';

export const RazorpayWebhookPage: React.FC = () => {
  const { isLive, isDemo, setMode, modeVersion } = useDatabaseMode();
  const [transactions, setTransactions] = useState<SimulationTransaction[]>([]);
  const [selectedTxn, setSelectedTxn] = useState<SimulationTransaction | null>(null);
  const [isLoadingTxns, setIsLoadingTxns] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Modal / Dispute Creation States
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [reasonCode, setReasonCode] = useState<string>('product_not_received');
  const [customReasonDescription, setCustomReasonDescription] = useState<string>('');
  const [disputeDescription, setDisputeDescription] = useState<string>(
    'Customer claims physical goods were never delivered by courier service.'
  );
  const [disputePhase, setDisputePhase] = useState<string>('chargeback');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [modalError, setModalError] = useState<string | null>(null);

  // Success / Delivery Result State
  const [deliveryResult, setDeliveryResult] = useState<SimulateDisputeResponse | null>(null);
  const [deliveryFailedError, setDeliveryFailedError] = useState<string | null>(null);

  const navigate = useNavigate();

  // Real-time event listener for live database events
  useRealtimeEvents({
    enabled: true,
    onRefresh: () => {
      loadTransactions();
    },
  });

  // Load available transactions from Live backend
  const loadTransactions = async () => {
    try {
      setIsLoadingTxns(true);
      setError(null);
      const list = await simulationService.getAvailableTransactions();

      const eligible = list.filter((t) => {
        const status = (t.transaction_status || '').toUpperCase();
        const hasDispute = Boolean(t.has_active_simulated_dispute || (t as any).has_active_dispute);
        return (status === 'SUCCESS' || status === 'CAPTURED') && !hasDispute;
      });

      const available = eligible.length > 0 ? eligible : list;
      setTransactions(available);

      if (available.length > 0 && !selectedTxn) {
        setSelectedTxn(available[0]);
      } else if (available.length === 0) {
        setSelectedTxn(null);
      }
    } catch (err: any) {
      setError(err.message || 'Failed to fetch live transactions from backend.');
    } finally {
      setIsLoadingTxns(false);
    }
  };

  useEffect(() => {
    loadTransactions();
  }, [modeVersion]);

  const handleReasonChange = (reason: string) => {
    setReasonCode(reason);
    if (reason === 'product_not_received') {
      setDisputeDescription('Customer claims physical goods were never delivered by courier service.');
    } else if (reason === 'fraudulent_transaction') {
      setDisputeDescription('Cardholder reports unrecognized charge; card was in their physical possession.');
    } else if (reason === 'duplicate_charge') {
      setDisputeDescription('Customer account was billed twice for the same transaction order.');
    } else if (reason === 'credit_not_processed') {
      setDisputeDescription('Merchant promised a return refund 14 days ago but funds have not credited.');
    } else if (reason === 'product_unacceptable') {
      setDisputeDescription('Received goods are severely damaged, defective, or not as described.');
    } else if (reason === 'other') {
      setDisputeDescription('');
    }
  };

  const handleOpenModalForTxn = (txn: SimulationTransaction, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    setSelectedTxn(txn);
    setReasonCode('product_not_received');
    setDisputeDescription('Customer claims physical goods were never delivered by courier service.');
    setCustomReasonDescription('');
    setDisputePhase('chargeback');
    setModalError(null);
    setIsModalOpen(true);
  };

  // Generate dynamic webhook payload preview
  const generatePayloadPreview = () => {
    if (!selectedTxn) return {};
    const description =
      reasonCode === 'other'
        ? customReasonDescription.trim() || 'Custom dispute reason specified by cardholder'
        : disputeDescription.trim();

    return {
      event: 'dispute.created',
      event_id: `evt_live_${Date.now().toString(36)}`,
      created_at: Math.floor(Date.now() / 1000),
      payload: {
        dispute: {
          entity: 'dispute',
          transaction_id: selectedTxn.transaction_id,
          amount: selectedTxn.amount,
          currency: selectedTxn.currency || 'INR',
          reason_code: reasonCode,
          description: description,
          phase: disputePhase,
          status: 'open',
          customer_id: selectedTxn.customer_id,
        },
      },
    };
  };

  const handleSendWebhook = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedTxn) return;

    if (reasonCode === 'other' && !customReasonDescription.trim()) {
      setModalError('Description is required when selecting "Other" dispute reason.');
      return;
    }

    try {
      setIsSubmitting(true);
      setModalError(null);
      setDeliveryFailedError(null);

      const description =
        reasonCode === 'other'
          ? customReasonDescription.trim()
          : disputeDescription.trim();

      const payload = {
        transaction_id: selectedTxn.transaction_id,
        reason_code: reasonCode,
        reason_description: description,
        phase: disputePhase,
      };

      // Call real webhook / dispute creation API
      const result = await simulationService.simulateDispute(payload);

      setIsModalOpen(false);
      setDeliveryResult(result);
      await loadTransactions();
    } catch (err: any) {
      const errMsg = err.message || 'Webhook processing failed.';
      setModalError(errMsg);
      setDeliveryFailedError(errMsg);
    } finally {
      setIsSubmitting(false);
    }
  };

  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans">
      {/* 1. Standalone Top Navigation Header */}
      <header className="h-16 bg-slate-900 border-b border-slate-800 px-6 flex items-center justify-between text-white shrink-0 select-none z-30 shadow-md">
        <div className="flex items-center gap-3.5">
          <div className="w-8 h-8 rounded-lg bg-indigo-500 flex items-center justify-center text-white font-bold text-base shadow-sm">
            R
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="font-bold text-sm tracking-tight text-white">
                Razorpay Webhook Simulator
              </span>
              <span className="px-2 py-0.5 text-[10px] font-bold rounded-md bg-indigo-950 text-indigo-300 border border-indigo-700/60 font-mono">
                EXTERNAL GATEWAY SIMULATOR
              </span>
            </div>
            <p className="text-[11px] text-slate-400">
              Live Backend Webhook Engine for AI Risk Manager
            </p>
          </div>
        </div>

        {/* Right Action: Mode Pill & Return to Merchant Portal */}
        <div className="flex items-center gap-3">
          <span
            className={`px-2.5 py-1 text-xs font-bold rounded-full border ${
              isLive
                ? 'bg-emerald-950 text-emerald-300 border-emerald-700/70'
                : 'bg-slate-800 text-slate-300 border-slate-700'
            }`}
          >
            {isLive ? '● LIVE BACKEND' : '● DEMO MODE'}
          </span>

          <Link
            to="/"
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-slate-200 hover:text-white bg-slate-800 hover:bg-slate-700 border border-slate-700 transition shadow-2xs"
          >
            <span>&larr; Back to Merchant Portal</span>
          </Link>
        </div>
      </header>

      {/* Main Content Area */}
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 space-y-4">
        {/* Page Title & Subtitle */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-slate-200">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">
              Razorpay Webhook
            </h1>
            <p className="text-xs text-slate-500 mt-0.5">
              Simulate Razorpay payment and dispute webhook events against the live application backend.
            </p>
          </div>
        </div>

        {/* Demo Mode Notice */}
        {isDemo && (
          <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-900 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xs">
            <div className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-amber-500 shrink-0" />
              <span>
                <strong>Demo Mode:</strong> Switch to <strong>Live Mode</strong> to simulate live chargebacks and trigger full automated AI analysis in the live database.
              </span>
            </div>
            <Button
              onClick={() => setMode('LIVE')}
              variant="primary"
              size="sm"
              className="bg-amber-600 hover:bg-amber-700 text-white shrink-0 font-semibold"
            >
              Switch to Live Mode &rarr;
            </Button>
          </div>
        )}

        {/* Webhook Delivery Result Banner (Success State) */}
        {deliveryResult && (
          <Card className="p-5 bg-emerald-50/80 border-emerald-300 space-y-3 shadow-2xs animate-in fade-in duration-200">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold text-sm shrink-0">
                  ✓
                </div>
                <div>
                  <h3 className="text-sm font-bold text-emerald-950">Webhook Delivered Successfully</h3>
                  <p className="text-xs text-emerald-800">
                    Live dispute created and registered in the database. Full AI & ML analysis was executed.
                  </p>
                </div>
              </div>

              <button
                type="button"
                onClick={() => setDeliveryResult(null)}
                className="text-emerald-700 hover:text-emerald-900 p-1 text-xs font-bold cursor-pointer"
              >
                ✕
              </button>
            </div>

            <div className="p-3.5 bg-white rounded-xl border border-emerald-200 text-xs grid grid-cols-2 sm:grid-cols-4 gap-3 text-slate-700">
              <div>
                <span className="text-[10px] text-slate-400 block font-mono">Dispute ID:</span>
                <strong className="font-mono text-slate-900">{deliveryResult.dispute_id}</strong>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 block font-mono">Transaction ID:</span>
                <strong className="font-mono text-slate-900">{deliveryResult.transaction_id}</strong>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 block font-mono">ML Analysis:</span>
                <span className="text-emerald-700 font-bold">
                  {deliveryResult.analysis_summary?.win_probability !== undefined
                    ? `${Math.round(deliveryResult.analysis_summary.win_probability * 100)}% Win Prob`
                    : 'Completed'}
                </span>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 block font-mono">Recommendation:</span>
                <span className="text-indigo-700 font-bold">
                  {deliveryResult.analysis_summary?.recommendation || 'CONTEST'}
                </span>
              </div>
            </div>

            <div className="flex items-center justify-end gap-2.5 pt-1">
              <Button
                onClick={() => setDeliveryResult(null)}
                variant="outline"
                size="sm"
                className="bg-white border-emerald-300 text-emerald-900 text-xs"
              >
                Dismiss
              </Button>
              <Button
                onClick={() => navigate(`/disputes/${deliveryResult.dispute_id}`)}
                variant="primary"
                size="sm"
                className="bg-emerald-600 hover:bg-emerald-700 font-semibold text-xs shadow-2xs"
              >
                Open Dispute Workspace &rarr;
              </Button>
            </div>
          </Card>
        )}

        {/* Webhook Delivery Failed Error Banner */}
        {deliveryFailedError && !deliveryResult && (
          <Card className="p-4 bg-rose-50 border-rose-300 space-y-2 shadow-2xs">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-rose-900 font-bold text-xs">
                <span>⚠️ Webhook Delivery Failed</span>
              </div>
              <button
                type="button"
                onClick={() => setDeliveryFailedError(null)}
                className="text-rose-700 hover:text-rose-900 p-1 text-xs font-bold cursor-pointer"
              >
                ✕
              </button>
            </div>
            <p className="text-xs text-rose-800">
              <strong>Reason:</strong> {deliveryFailedError}
            </p>
          </Card>
        )}

        {/* Main Webhook Event / Transaction Area */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 flex-1 items-start">
          {/* Left Column: Available Live Transactions List (7 cols) */}
          <div className="lg:col-span-7 space-y-2.5">
            <div className="flex items-center justify-between">
              <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider font-mono">
                Live Transaction List ({transactions.length})
              </h2>
              <span className="text-[11px] text-slate-400">Click [Raise Dispute] on any transaction</span>
            </div>

            {isLoadingTxns ? (
              <div className="space-y-2.5">
                <Skeleton className="h-24 w-full" />
                <Skeleton className="h-24 w-full" />
                <Skeleton className="h-24 w-full" />
              </div>
            ) : error ? (
              <Card className="p-6 text-center text-rose-600 text-xs border-rose-200">
                {error}
              </Card>
            ) : transactions.length === 0 ? (
              <Card className="p-8 text-center text-xs text-slate-500 bg-white border-slate-200">
                No eligible transactions available in the Live backend.
              </Card>
            ) : (
              <div className="space-y-2.5 max-h-[calc(100vh-270px)] overflow-y-auto pr-1">
                {transactions.map((t) => {
                  const isSelected = selectedTxn?.transaction_id === t.transaction_id;

                  return (
                    <div
                      key={t.transaction_id}
                      onClick={() => setSelectedTxn(t)}
                      className={`p-4 rounded-xl border transition-all cursor-pointer select-none bg-white ${
                        isSelected
                          ? 'border-indigo-600 ring-2 ring-indigo-500/20 shadow-xs'
                          : 'border-slate-200 hover:border-slate-300 hover:shadow-2xs'
                      }`}
                    >
                      <div className="flex items-center justify-between gap-3">
                        {/* Radio Indicator & Transaction ID */}
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div
                            className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 ${
                              isSelected
                                ? 'border-indigo-600 bg-indigo-600'
                                : 'border-slate-300 bg-white'
                            }`}
                          >
                            {isSelected && <div className="w-1.5 h-1.5 rounded-full bg-white" />}
                          </div>
                          <div className="min-w-0">
                            <span className="font-mono font-bold text-xs text-slate-900 truncate block">
                              {t.transaction_id}
                            </span>
                            <span className="text-[11px] text-slate-500 truncate block">
                              Customer: {t.customer_name || t.customer_id}
                            </span>
                          </div>
                        </div>

                        {/* Amount & Status */}
                        <div className="text-right shrink-0">
                          <div className="font-bold text-xs text-slate-900">
                            {formatCurrency(t.amount, t.currency)}
                          </div>
                          <div className="flex items-center justify-end gap-1.5 mt-0.5">
                            <span className="text-[10px] font-mono text-slate-500 uppercase">
                              {t.payment_method}
                            </span>
                            <span className="px-1.5 py-0.2 text-[9px] font-bold rounded bg-emerald-50 text-emerald-700 border border-emerald-200">
                              {t.transaction_status || 'Captured'}
                            </span>
                          </div>
                        </div>
                      </div>

                      <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-[10px] text-slate-500">
                        <span className="font-mono">Country: {t.transaction_country || t.country || 'IN'} · {formatDate(t.timestamp)}</span>
                        
                        <Button
                          onClick={(e) => handleOpenModalForTxn(t, e)}
                          variant="primary"
                          size="sm"
                          className="bg-rose-600 hover:bg-rose-700 text-white font-bold text-[11px] px-2.5 py-1 shadow-xs"
                        >
                          Raise Dispute
                        </Button>
                      </div>
                    </div>
                  );
                })}
              </div>
            )}
          </div>

          {/* Right Column: Event Configuration & Webhook Payload Preview (5 cols) */}
          <div className="lg:col-span-5 space-y-3 sticky top-4">
            <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider font-mono">
              Webhook Event Configuration
            </h2>

            {selectedTxn ? (
              <Card className="p-4 space-y-4 bg-white border-slate-200 shadow-2xs">
                <div>
                  <span className="text-[10px] font-mono text-indigo-600 font-bold uppercase tracking-wider">
                    Selected Transaction
                  </span>
                  <h3 className="font-mono font-bold text-sm text-slate-900 mt-0.5">
                    {selectedTxn.transaction_id}
                  </h3>
                </div>

                {/* Transaction Summary Details */}
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/80 space-y-2 text-xs">
                  <div className="flex justify-between py-1 border-b border-slate-200/60">
                    <span className="text-slate-500">Amount</span>
                    <strong className="text-slate-900 font-semibold">
                      {formatCurrency(selectedTxn.amount, selectedTxn.currency)}
                    </strong>
                  </div>
                  <div className="flex justify-between py-1 border-b border-slate-200/60">
                    <span className="text-slate-500">Customer Account</span>
                    <span className="text-slate-800 font-mono">{selectedTxn.customer_name || selectedTxn.customer_id}</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-slate-200/60">
                    <span className="text-slate-500">Payment Method</span>
                    <span className="text-slate-800 uppercase font-mono font-medium">{selectedTxn.payment_method}</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-slate-200/60">
                    <span className="text-slate-500">Transaction Status</span>
                    <span className="text-emerald-700 font-semibold">{selectedTxn.transaction_status || 'CAPTURED'}</span>
                  </div>
                  <div className="flex justify-between py-1">
                    <span className="text-slate-500">Created Date</span>
                    <span className="text-slate-700 font-mono text-[11px]">{formatDate(selectedTxn.timestamp)}</span>
                  </div>
                </div>

                {/* Webhook Payload JSON Preview */}
                <div className="space-y-1">
                  <span className="text-[10px] font-mono text-slate-500 uppercase font-bold block">
                    Live Webhook Payload (dispute.created)
                  </span>
                  <pre className="p-3 bg-slate-900 text-indigo-300 font-mono text-[11px] rounded-xl overflow-x-auto leading-relaxed border border-slate-800 max-h-48">
                    {JSON.stringify(generatePayloadPreview(), null, 2)}
                  </pre>
                </div>

                {/* Action: Open Raise Dispute Dialog */}
                <div className="pt-2">
                  <Button
                    onClick={(e) => handleOpenModalForTxn(selectedTxn, e)}
                    variant="primary"
                    size="md"
                    className="w-full bg-rose-600 hover:bg-rose-700 font-bold text-xs shadow-xs py-2.5"
                  >
                    Raise Dispute &rarr;
                  </Button>
                  <p className="text-[10px] text-slate-400 text-center mt-2">
                    Submits dispute to backend and triggers automatic AI+ML analysis.
                  </p>
                </div>
              </Card>
            ) : (
              <Card className="p-6 text-center text-xs text-slate-500 bg-white border-slate-200">
                Select a transaction card on the left to configure the webhook event and raise a dispute.
              </Card>
            )}
          </div>
        </div>

        {/* POPUP DIALOG: Raise Dispute / Send Webhook Modal */}
        <Modal
          isOpen={isModalOpen}
          onClose={() => setIsModalOpen(false)}
          title="Raise Live Dispute"
          subtitle="Select dispute reason and confirm details. Backend creates the dispute in live database."
          footer={
            <>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setIsModalOpen(false)}
                disabled={isSubmitting}
              >
                Cancel
              </Button>
              <Button
                variant="primary"
                size="sm"
                isLoading={isSubmitting}
                onClick={handleSendWebhook}
                className="bg-rose-600 hover:bg-rose-700 font-bold shadow-xs"
              >
                Confirm & Raise Dispute
              </Button>
            </>
          }
        >
          <form onSubmit={handleSendWebhook} className="space-y-4 text-xs">
            {modalError && (
              <div className="p-3 bg-rose-50 border border-rose-200 text-rose-800 rounded-lg text-xs">
                {modalError}
              </div>
            )}

            {/* Selected Transaction Summary */}
            {selectedTxn && (
              <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 text-slate-700 space-y-1.5">
                <div className="flex justify-between">
                  <span className="text-slate-400 font-mono text-[10px]">Selected Transaction:</span>
                  <span className="font-mono font-bold text-slate-900">{selectedTxn.transaction_id}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400 font-mono text-[10px]">Amount:</span>
                  <span className="font-bold text-slate-900">{formatCurrency(selectedTxn.amount, selectedTxn.currency)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400 font-mono text-[10px]">Customer:</span>
                  <span className="text-slate-800 font-medium">{selectedTxn.customer_name || selectedTxn.customer_id}</span>
                </div>
              </div>
            )}

            {/* Dispute Reason Selection (Radio Options) */}
            <div className="space-y-2">
              <label className="block font-bold text-slate-900 text-xs">
                Dispute Reason *
              </label>
              <div className="space-y-1.5">
                {[
                  { id: 'product_not_received', label: 'Product Not Received' },
                  { id: 'fraudulent_transaction', label: 'Fraudulent Transaction' },
                  { id: 'duplicate_charge', label: 'Duplicate Charge' },
                  { id: 'product_unacceptable', label: 'Product Unacceptable' },
                  { id: 'credit_not_processed', label: 'Credit Not Processed' },
                  { id: 'other', label: 'Other' },
                ].map((r) => {
                  const isChecked = reasonCode === r.id;
                  return (
                    <label
                      key={r.id}
                      onClick={() => handleReasonChange(r.id)}
                      className={`flex items-center gap-2.5 p-2.5 rounded-lg border cursor-pointer transition select-none ${
                        isChecked
                          ? 'border-indigo-600 bg-indigo-50/50 text-indigo-950 font-semibold'
                          : 'border-slate-200 hover:bg-slate-50 text-slate-700'
                      }`}
                    >
                      <input
                        type="radio"
                        name="dispute_reason"
                        checked={isChecked}
                        onChange={() => handleReasonChange(r.id)}
                        className="text-indigo-600 focus:ring-indigo-500 h-3.5 w-3.5"
                      />
                      <span>{r.label}</span>
                    </label>
                  );
                })}
              </div>
            </div>

            {/* Dispute Phase Selection */}
            <div className="space-y-1">
              <label className="block font-medium text-slate-700 text-xs">
                Dispute Phase
              </label>
              <select
                value={disputePhase}
                onChange={(e) => setDisputePhase(e.target.value)}
                className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 text-slate-900 text-xs font-mono"
              >
                <option value="chargeback">chargeback</option>
                <option value="retrieval">retrieval</option>
                <option value="pre_arbitration">pre_arbitration</option>
                <option value="arbitration">arbitration</option>
                <option value="fraud">fraud</option>
              </select>
            </div>

            {/* Description Textarea */}
            {reasonCode === 'other' ? (
              <div className="space-y-1.5 p-3 bg-amber-50/60 rounded-xl border border-amber-200">
                <label className="block font-bold text-amber-950 text-xs">
                  Description * (Required for 'Other')
                </label>
                <textarea
                  rows={2}
                  required
                  placeholder="Describe the custom chargeback reason..."
                  value={customReasonDescription}
                  onChange={(e) => setCustomReasonDescription(e.target.value)}
                  className="w-full bg-white border border-amber-300 rounded-lg p-2 text-slate-900 focus:outline-none focus:ring-2 focus:ring-amber-500/30 text-xs"
                />
              </div>
            ) : (
              <div className="space-y-1">
                <label className="block font-medium text-slate-700 text-xs">
                  Dispute Description / Notes
                </label>
                <textarea
                  rows={2}
                  value={disputeDescription}
                  onChange={(e) => setDisputeDescription(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 text-slate-900 text-xs leading-relaxed"
                />
              </div>
            )}
          </form>
        </Modal>
      </main>
    </div>
  );
};
